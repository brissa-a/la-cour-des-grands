...
async function getScrutinIdScrutinNameIndex(opt) {
  const destfile = [opt.dlfolder, "scrutins.json"].join('/')
  if (!fs.existsSync(destfile) || opt.override) {
...
async function fetchOrganeActeur(uid, opt) {
  const {dlfolder} = opt
  await downloadUnzip(acteurEtOrgane.url, [dlfolder, acteurEtOrgane.dest].join("/"), opt)
  let opendatapath = dlfolder + "/data.assemblee-nationale.fr/" + `/acteurOrgane/json/acteur/${uid}.json`;
...
module.exports = {fetchAnData: fetchOrganeActeur, fetchAnDataOf, fetchVoteOf, getScrutinIdScrutinNameIndex}
